...
const AboutUs = () => {
  ...
  return (
    <section className="relative w-full min-h-screen bg-gradient-to-br from-green-50 to-white overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-96 h-96 bg-green-300 rounded-full opacity-20 blur-3xl animate-pulse-slow z-0" />
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-lime-200 rounded-full opacity-20 blur-2xl animate-pulse-slow z-0" />

      <div
        ref={ref}
        className={`about-container max-w-7xl mx-auto px-6 py-24 flex flex-col-reverse md:flex-row gap-14 items-center z-10 relative fade-slide-up ${isVisible ? 'visible' : ''}`}
      >
        {/* Left: Text Content */}
        <div className="w-full md:w-1/2 space-y-6 animate-fade-in-up delay-200">
          ...
          <p className="text-gray-700 leading-relaxed word-reveal text-lg animate-fade-in-up delay-300">
            Nestled amidst nature’s finest, <span className="font-semibold http://localhost:5173/ text-green-700">Kingsukh Guest House</span> offers an escape like no other — surrounded by the majestic Baranti, Susunia, and Biharinath hills.
        ...
        <div className="w-full md:w-1/2 relative rounded-xl shadow-xl overflow-hidden animate-zoom-in">
          <img
